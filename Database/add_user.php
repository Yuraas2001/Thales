<?php
    ...
    if (isset($_POST['username'], $_POST['password'], $_POST['role'])) {
        ...
        if (!preg_match('/^[^\p{M}]*$/u', $password)) {
            echo "Le mot de passe ne doit pas contenir d’accent.";
        } elseif (strpos($password, $username) !== false) {
            echo "Le mot de passe ne doit pas contenir le login de l’utilisateur.";
        } elseif (!preg_match('/^[a-zA-Z0-9[!"#$%&\'*+,-./;<=>?@\\\^_`|}~],{}]*$/', $password)) {
            echo "Le mot de passe contient des caractères non autorisés.";
        } elseif (preg_match_all('/[0-9]/', $password) < $n) {
            echo "Le mot de passe doit contenir au moins $n caractère(s) numérique(s).";
        } elseif (preg_match_all('/[a-z]/', $password) < $p) {
            echo "Le mot de passe doit contenir au moins $p caractère(s) alphabétique(s) en minuscule.";
        } elseif (preg_match_all('/[A-Z]/', $password) < $q) {
            echo "Le mot de passe doit contenir au moins $q caractère(s) alphabétique(s) en majuscule.";
        } elseif (preg_match_all('/[!"#$%&\'*+,-./;<=>?@\\\^_`|}~],{}/', $password) < $r) {
            echo "Le mot de passe doit contenir au moins $r caractère(s) spécial(aux).";
        } else {
